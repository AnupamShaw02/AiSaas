...
const GenerateImages = () => {
  ...
  const imageStyle = [ 'Realistic', 'Ghibli style','Anime style','Cartoon style','Fantasy style','Realistic style','3D style','Portrait style' ]
      ...
      const onSubmitHandler = async (e)=>{
        ...
        try {
          setLoading(true)
          const prompt = `Generate an image about ${input} in the ${selectedStyle} style.`
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-xl'>
          {imageStyle.map((item)=> (
            ...
            className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle === item ? 'bg-green-50 text-green-700 border-green-700' : 
